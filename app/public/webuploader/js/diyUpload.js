...
(function($) {
  ...
  $.fn.extend({
    ...
    diyUpload(opt, serverCallBack) {
      ...
      const webUploader = getUploader(opt);
      ...
      webUploader.on('uploadProgress', function(file, percentage) {
        const $fileBox = $('#fileBox_' + file.id);
        const $diyBar = $fileBox.find('.diyBar');
        $diyBar.show();
      ...
      webUploader.on('uploadError', function(file, reason) {
        ...
        const err = '上传失败! 文件:' + file.name + ' 错误码:' + reason;
        if (errorCallBack) {
          errorCallBack(err);
        }
      });
  ...
  function showDiyProgress(progress, $diyBar, text) {
    ...
    const $diyProgress = $diyBar.find('.diyProgress');
    const $diyProgressText = $diyBar.find('.diyProgressText');
  ...
  function createBox($fileInput, file, webUploader) {
    ...
    let $parentFileBox = $fileInput.next('.parentFileBox');
    ...
    if ($parentFileBox.find('.diyButton').length <= 0) {
      ...
      const $startButton = $parentFileBox.find('.diyStart');
      const $cancelButton = $parentFileBox.find('.diyCancelAll');
      ...
      $cancelButton.bind('click', function() {
        const fileArr = webUploader.getFiles('queued');
    ...
    // 添加子容器;
    const li = '<li id="fileBox_' + file_id + '" class="diyUploadHover"> \
    ...
    // 父容器宽度;
    let $width = $('.fileBoxUl>li').length * 180;
    const $maxWidth = $fileInput.parent().width();
    $width = $maxWidth > $width ? $width : $maxWidth;
    $parentFileBox.width($width);

    const $fileBox = $parentFileBox.find('#fileBox_' + file_id);

    // 绑定取消事件;
    const $diyCancel = $fileBox.children('.diyCancel').one('click', function() {
      removeLi($(this).parent('li'), file_id, webUploader);
    });

    if (file.type.split('/')[0] != 'image') {
      const liClassName = getFileTypeClassName(file.name.split('.').pop());
